...
def processVideo(videoPath):
    ...
    while cap.isOpened():
        ...
        boxes = results.xyxy[0].numpy()     # Bounding Box
        labels = results.names      # Class
    ...
    for box_name, boxInfo in BB.items():
        print(f"{box_name}: {boxInfo['flag']}")


# Process video from specified path or replace with camera feed for real-time processing
processVideo('Media/.gitinclude')